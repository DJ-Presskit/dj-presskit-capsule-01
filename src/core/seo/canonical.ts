 ...
 *
 * Priority:
 * 1. x-tenant-host (router provides this for proxied DJ domains)
 * 2. x-canonical-host (legacy/alternative)
 * 3. x-forwarded-proto + x-forwarded-host
 * 4. host header
 * 5. NEXT_PUBLIC_SITE_URL env variable
 * 6. localhost fallback for dev
 */
export async function getCanonicalOrigin(): Promise<string> {
  const headersList = await headers();

  // 1. Check for router-provided tenant host (proxied DJ domains)
  ...
  if (tenantHost) {
    ...
    return `https://${tenantHost}`.replace(/\/$/, "");
  }

  // 2. Check for x-canonical-host (legacy/alternative)
 ...
 */
export function buildCanonicalBaseUrl({ origin, slug, lang }: CanonicalBaseUrlParams): string {
